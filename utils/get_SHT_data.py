...
@log_function_name
def get_DTRlaser_times(sht_number: int, sht_path: str,
                       laser_frequency: int = 100,
                       signal_threshold: float = 0.25,
                       max_laser_shots_number: int = 23) -> tuple:
    """
        ...
        for time, signal in zip(x_data, y_data):
            if signal > signal_threshold:
                if not DTR_times:
                    ...
                    idle_time = time + 1 / laser_frequency * 0.7  # seconds, 0.8 - in case laser instability
                elif time < idle_time:
                    pass
                else:
                    DTR_times.append(round(time * 1E3, 2))  # seconds to ms
                    idle_time = time + 1 / laser_frequency * 0.7
                    if DTR_times[-2] - DTR_times[-1] > 1E3 / laser_frequency * 1.2:  # 1.2 - in case laser instability
