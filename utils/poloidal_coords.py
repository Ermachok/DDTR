...
def get_magnetic_equilibrium_data(
    ...
    try:
        ...
        return {"z_m": z_data, "r_m": r_data, "psi": psi_data}
...
def prepare_data_for_poloidal_plot(ts_data: List[Dict], equilibrium: Dict) -> Tuple:
    ...
    ts_nt_err = [
        (te_err / te + ne_err / ne) * ne * te if te != 0 and ne != 0 else 0
        ...
        for time in [160.6]:
            ...
            if equilibrium_data:
                dts_time_idx, nearest_dts_shot = find_nearest(
                    np.array(dts_data["times_ms"]), time, out_value=True, out_index=True
                )
                nearest_ets_shot = find_nearest(
                    np.array(equator_data["times_ms"]), time, out_value=True
                )
                ...
                dts_points = [
                    ...
                    for z, ne, Te, ne_err, Te_err in zip(
                        dts_data["Z_m"],
                        dts_data["ne(t)"],
                        dts_data["Te(t)"],
                        dts_data["ne_err(t)"],
