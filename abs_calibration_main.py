...
if __name__ == "__main__":
    ...
    combiscope_times, fibers = built_fibers(
        laser_shot_num, config, calibration_flag=True
    )

    absolut_calibration = calculate_calibration_coef(
        fibers,
        ophir_data_path=config["path_ophir_calibration"],
    ...
    with open(
        "calibrations/calibration_configs/absolute_calib_june2024_v3.json", "w"
    ) as file:
