...
def draw_separatrix(
    ...
    if ax is None:
        ...
        ax.plot(
            separatrix_data["body"]["R"],
            separatrix_data["body"]["Z"],
            color,
            label=time,
        )
        ...
        ax.plot(
            separatrix_data["body"]["R"], separatrix_data["body"]["Z"], "-b", label=time
        )
...
def draw_ir_camera(shot_num: str, ir_data: dict, ax):
    ...
    ax.plot(ir_data["radii"], [0 for _ in range(len(ir_data["radii"]))], label=shot_num)
...
def draw_distance_from_separatrix_originVersion(
    ...
    axs[0].plot(
        equator_distances, ne_equator, "o-", label=f"eqTS, {nearest_equator_time}"
    )
    axs[0].plot(
        dts_distances, ne_dts, "o-", label=f"DTS, {dts_data['t'][index_dts_time]}"
    )
    ...
    axs[1].plot(
        equator_distances, te_equator, "o-", label=f"eqTS, {nearest_equator_time}"
    )
    axs[1].plot(
        dts_distances, te_dts, "o-", label=f"DTS, {dts_data['t'][index_dts_time]}"
    )
